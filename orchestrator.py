...
class Orchestrator:
    def __init__(self, config: SystemConfig):
        ...
        self._running_procs: Dict[str, object] = {}  # subprocess.Popen or _DetachedProcess
    ...
    def run(self, max_iterations: int = 50):
        ...
        for iteration in range(self.state.iteration, max_iterations):
            ...
            proposals = self.engineer.propose(
                ...
                current_best_mae=current_best_mae,
                iteration=iteration,
                num_proposals=min(3, self.config.max_parallel),
            )
            ...
            for proposal in proposals:
                ...
                if proposal.get("code_change_type") != "hyperparameter" and proposal.get("changes"):
                    success = self.engineer.implement(
                        proposal, code_dir,
                        baseline_code_dir=self.config.scripts_dir,
                    )
                    if not success:
        ...
        while pending:
            ...
            for exp_id in pending:
                ...
                if proc is None or proc.poll() is not None:
                    ...
                    # For _DetachedProcess, check exit via training.log / results
                    # returncode is only available on subprocess.Popen
